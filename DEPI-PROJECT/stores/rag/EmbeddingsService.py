...
class EmbeddingsService:
    ...
    def _initialize_provider(self):
        ...
        try:
            ...
            self.provider = factory.create(provider=self.config.GENERATION_BACKEND)

            # Set embedding model
            if hasattr(self.config, 'OPENAI_EMBEDDING_MODEL') and self.config.GENERATION_BACKEND == "openai":
                self.provider.set_embedding_model(
                    model_id=self.config.OPENAI_EMBEDDING_MODEL,
                    embedding_size=1536 
                )
            elif hasattr(self.config, 'COHERE_EMBEDDING_MODEL') and self.config.GENERATION_BACKEND == "cohere":
                self.provider.set_embedding_model(
                    model_id=self.config.COHERE_EMBEDDING_MODEL,
                    embedding_size=1024 
                )
            elif hasattr(self.config, 'OLLAMA_EMBEDDING_MODEL') and self.config.GENERATION_BACKEND == "OLLAMA":
                self.provider.set_embedding_model(
                    model_id=self.config.OLLAMA_EMBEDDING_MODEL,
                    embedding_size=768  
                )    

            self.logger.info(f"Embeddings provider initialized: {self.config.GENERATION_BACKEND}")
    ...
    def get_embedding_dimension(self) -> int:
        ...
        if not self.provider or not hasattr(self.provider, 'embedding_size'):
            ...
            if self.config.GENERATION_BACKEND == "openai":
                return 1536
            elif self.config.GENERATION_BACKEND == "cohere":
                return 1024
            else:
                return 768
